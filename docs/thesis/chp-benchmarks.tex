...
take too long. For instance, when running inference on $k$-GD, average prediction took about $0.1$
...
impressive results, with accuracies reaching $>$98\%. Despite these results, a model that takes too
long for inference is not adequate for a self-driving application. For this reason, we show only
comparisons with the $k$-GD model.
\begin{figure}
  \centering
  \begin{tikzpicture}
  ...
  \end{tikzpicture}
  \caption{Our training pipeline when using binarization in pre-processing.}
\end{figure}
...
\begin{figure}[h]
  ...
  \end{tikzpicture}
  \caption{Our inference pipeline when using binarization in pre-processing.}
\end{figure}
